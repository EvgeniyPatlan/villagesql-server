...
// VillageSQL auto-create: if a VEF auth method opted in to unknown accounts,
// point this decoy at the method's plugin, make it authenticatable, and flag it
// a provisioning candidate -- instead of the anti-enumeration decoy. Mutates
// the non-const `decoy` before it becomes the const mpvio->acl_user.
...
// VillageSQL auto-create: for a provisioning login (an unknown-account decoy),
...
int acl_authenticate(THD *thd, enum_server_command command) {
  ...
  */
  ...
  // VillageSQL auto-create: now that the handler accepted the login, provision
