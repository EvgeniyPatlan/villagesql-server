...
/// Contains all the basic tuple-like interfaces \forcedlink{get},
/// \forcedlink{tuple_size}, \forcedlink{tuple_element_t}, and others.
///
/// \b Synopsis:
...
/// \brief Returns reference or const reference to a field with index `I` in
/// \aggregate `val`. Overload taking the type `U` returns reference or const
/// reference to a field with provided type `U` in \aggregate `val` if there's
/// only one field of such type in `val`.
///
/// \b Example:
/// \code
...
/// \overload get
template <std::size_t I, class T>
constexpr auto get(T &, std::enable_if_t<!std::is_assignable<T, T>::value> * =
                            nullptr) noexcept {
  static_assert(sizeof(T) && false,
                "====================> Boost.PFR: Calling boost::pfr::get on "
                "non const non assignable type is allowed only in C++17");
  return 0;
}
#endif
...
constexpr const U &get(const T &val) noexcept {
  return detail::sequence_tuple::get_by_type_impl<const U &>(
...
constexpr U &get(T &val
  ...
  return detail::sequence_tuple::get_by_type_impl<U &>(
      detail::tie_as_tuple(val));
}
...
/// \overload get
template <class U, class T>
constexpr U &get(T &, std::enable_if_t<!std::is_assignable<T, T>::value> * =
                          nullptr) noexcept {
  static_assert(sizeof(T) && false,
                "====================> Boost.PFR: Calling boost::pfr::get on "
                "non const non assignable type is allowed only in C++17");
...
constexpr U &&get(T &&val,
                  std::enable_if_t<std::is_rvalue_reference<T &&>::value> * =
                      nullptr) noexcept {
  return std::move(
      detail::sequence_tuple::get_by_type_impl<U &>(detail::tie_as_tuple(val)));
}

/// \brief `tuple_element` has a member typedef `type` that returns the type of
/// a field with index I in \aggregate T.
///
/// \b Example:
/// \code
...
using tuple_element = detail::sequence_tuple::tuple_element<
    I, decltype(::boost::pfr::detail::tie_as_tuple(std::declval<T &>()))>;
...
///
/// \returns a `std::tuple` with lvalue and const lvalue references to fields of
/// an \aggregate `val`.
///
/// \b Example:
/// \code
...
constexpr auto structure_tie(
    T &,
    std::enable_if_t<!std::is_assignable<T, T>::value> * = nullptr) noexcept {
  static_assert(
      sizeof(T) && false,
      "====================> Boost.PFR: Calling boost::pfr::structure_tie on "
      "non const non assignable type is allowed only in C++17");
  return 0;
}
#endif
...
template <class T>
constexpr auto structure_tie(
    T &&, std::enable_if_t<std::is_rvalue_reference<T &&>::value> * =
              nullptr) noexcept {
  static_assert(sizeof(T) && false,
                "====================> Boost.PFR: Calling "
                "boost::pfr::structure_tie on rvalue references is forbidden");
  return 0;
}
...
///     * any_return_type func(U&& field)                // field of value is
///     perfect forwarded to function
///     * any_return_type func(U&& field, std::size_t i)
///     * any_return_type func(U&& value, I i)           // Here I is an
///     `std::integral_constant<size_t, field_index>`
///
...
///     struct my_struct { int i, short s; };
///     int sum = 0;
///     boost::pfr::for_each_field(my_struct{20, 22}, [&sum](const auto& field)
///     { sum += field; }); assert(sum == 42);
/// \endcode
...
/// \brief std::tie-like function that allows assigning to tied values from
/// aggregates.
///
/// \returns an object with lvalue references to `args...`; on assignment of an
/// \aggregate value to that object each field of an aggregate is assigned to
/// the corresponding `args...` reference.
///
/// \b Example:
/// \code
///     auto f() {
...
constexpr detail::tie_from_structure_tuple<Elements...> tie_from_structure(
    Elements &...args) noexcept {
