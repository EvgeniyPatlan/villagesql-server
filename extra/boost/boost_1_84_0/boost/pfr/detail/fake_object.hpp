...
// This variable serves as a compile-time assert. If you see any error here,
// then you're probably using `boost::pfr::get_name()` or
// `boost::pfr::names_as_array()` with a non-external linkage type.
template <class T>
extern const T passed_type_has_no_external_linkage;

// For returning non default constructible types, it's exclusively used in
