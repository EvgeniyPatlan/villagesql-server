...
//
// Description:
//  The Great Type Loophole is a technique that allows to exchange type
//  information with template instantiations. Basically you can assign and read
//  type information during compile time. Here it is used to detect data members
//  of a data type. I described it for the first time in this blog post
//  http://alexpolt.github.io/type-loophole.html .
//
// This technique exploits the
// http://www.open-std.org/jtc1/sc22/wg21/docs/cwg_active.html#2118 CWG 2118.
// Stateful metaprogramming via friend injection Note: CWG agreed that such
// techniques should be ill-formed, although the mechanism for prohibiting them
...
// There are two types: one with the auto return type, which is the way we read
// types later. The second one is used in the detection of instantiations
// without which we'd get multiple definitions.
...
struct fn_def_lref {
  friend auto loophole(tag<T, N>) {
    // Standard Library containers do not SFINAE on invalid copy constructor.
    // Because of that std::vector<std::unique_ptr<int>> reports that it is
    // copyable, which leads to an instantiation error at this place.
    //
    // To workaround the issue, we check that the type U is movable, and move it
    // in that case.
    using no_extents_t = std::remove_all_extents_t<U>;
    return static_cast<
        std::conditional_t<std::is_move_constructible<no_extents_t>::value,
                           no_extents_t &&, no_extents_t &>>(
...
struct fn_def_rref {
  friend auto loophole(tag<T, N>) {
    return std::move(
        boost::pfr::detail::unsafe_declval<std::remove_all_extents_t<U> &>());
  }
};
...
// instantiations. Important point, using sizeof seems to be more reliable. Also
// default template arguments are "cached" (I think). To fix that I provide a U
// template parameter to the ins functions which do the detection using
// constexpr friend functions and SFINAE.
template <class T, std::size_t N>
struct loophole_ubiq_lref {
  ...
  template <class U,
            std::size_t = sizeof(
                fn_def_lref<T, U, N, sizeof(ins<U, N>(0)) == sizeof(char)>)>
  constexpr operator U &()
      const && noexcept;  // `const&&` here helps to avoid ambiguity in loophole
                          // instantiations. optional_like test validate that
                          // behavior.
};
...
struct loophole_ubiq_rref {
  ...
  template <class U,
            std::size_t = sizeof(
                fn_def_rref<T, U, N, sizeof(ins<U, N>(0)) == sizeof(char)>)>
  constexpr operator U &&()
      const && noexcept;  // `const&&` here helps to avoid ambiguity in loophole
                          // instantiations. optional_like test validate that
                          // behavior.
...
auto tie_as_tuple_loophole_impl(T &lvalue) noexcept {
  ...
  using loophole_type_list = typename detail::loophole_type_list_selector<
      std::is_copy_constructible<std::remove_all_extents_t<type>>::value, type,
      indexes>::type;
  using tuple_type = typename loophole_type_list::type;

  return boost::pfr::detail::make_flat_tuple_of_references(
      lvalue, offset_based_getter<type, tuple_type>{}, size_t_<0>{},
...
auto tie_as_tuple(T &val) noexcept {
  ...
  return boost::pfr::detail::tie_as_tuple_loophole_impl(val);
}
...
void for_each_field_dispatcher(T &t, F &&f, std::index_sequence<I...>) {
  static_assert(!std::is_union<T>::value,
                ...
                "forbidden to reflect unions. See `Reflection of unions` "
                "section in the docs for more info.");
  std::forward<F>(f)(boost::pfr::detail::tie_as_tuple_loophole_impl(t));
}
