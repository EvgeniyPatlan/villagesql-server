...
struct core_name_skip {
  ...
  consteval std::string_view apply(std::string_view sv) const noexcept {
    ...
    const auto found =
        is_backward ? sv.rfind(until_runtime) : sv.find(until_runtime);
...
struct backward {
  explicit consteval backward(std::string_view value) noexcept : value(value) {}

  std::string_view value;
};

consteval core_name_skip make_core_name_skip(
    std::size_t size_at_begin, std::size_t size_at_end,
...
#define BOOST_PFR_FUNCTION_SIGNATURE ""
  ...
  constexpr std::string_view sv =
      detail::clang_workaround<MsvcWorkaround>(BOOST_PFR_FUNCTION_SIGNATURE);
  static_assert(!sv.empty(),
                "====================> Boost.PFR: Field reflection parser "
                "configured in a wrong way. "
                "Please define the BOOST_PFR_FUNCTION_SIGNATURE to a compiler "
                "specific macro, "
                "that outputs the whole function signature including non-type "
                "template parameters.");

  constexpr auto skip = detail::make_core_name_skip BOOST_PFR_CORE_NAME_PARSING;
  static_assert(
      skip.size_at_begin + skip.size_at_end + skip.until_runtime.size() <
          sv.size(),
      "====================> Boost.PFR: Field reflection parser configured in "
      "a wrong way. "
      "It attempts to skip more chars than available. "
      "Please define BOOST_PFR_CORE_NAME_PARSING to correct values. See "
      "documentation section "
      "'Limitations and Configuration' for more information.");
  constexpr auto fn = skip.apply(sv);
  static_assert(!fn.empty(),
                "====================> Boost.PFR: Extraction of field name is "
                "misconfigured for your compiler. "
                "It skipped all the input, leaving the field name empty. "
                "Please define BOOST_PFR_CORE_NAME_PARSING to correct values. "
...
// clang 16 and earlier don't support address of non-static member as template
// parameter but fortunately it's possible to use C++20 non-type template
// parameters in another way even in clang 16 and more older clangs all we need
// is to wrap pointer into 'clang_wrapper_t' and then pass it into template
template <class T>
...
constexpr const T &make_clang_wrapper(const T &arg) noexcept {
  // It's everything OK with address of non-static member as template parameter
  // support on this compiler so we don't need a wrapper here, just pass the
  // pointer into template
  return arg;
}
...
//
// Without passing 'T' into 'name_of_field' different fields from different
// structures might have the same name! See
// https://developercommunity.visualstudio.com/t/__FUNCSIG__-outputs-wrong-value-with-C/10458554
// for details
template <class T, std::size_t I>
inline constexpr auto stored_name_of_field = detail::name_of_field<
    T, detail::make_clang_wrapper(std::addressof(detail::sequence_tuple::get<I>(
...
constexpr auto tie_as_names_tuple_impl(std::index_sequence<I...>) noexcept {
  return detail::sequence_tuple::make_sequence_tuple(
      std::string_view{stored_name_of_field<T, I>.data()}...);
}
...
constexpr std::string_view get_name() noexcept {
  ...
  static_assert(!std::is_array<T>::value,
                "====================> Boost.PFR: It is impossible to extract "
                "name from old C array since it doesn't have named members");
  static_assert(sizeof(T) && BOOST_PFR_USE_CPP17,
                "====================> Boost.PFR: Extraction of field's names "
                "is allowed only when the BOOST_PFR_USE_CPP17 macro enabled.");
...
constexpr auto tie_as_names_tuple() noexcept {
  ...
  return detail::tie_as_names_tuple_impl<T>(
