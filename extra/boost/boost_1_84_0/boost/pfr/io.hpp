...
/// If type is streamable using its own operator or its conversion operator,
/// then the types operator is used.
///
/// \b Example:
/// \code
...
///     std::cout << boost::pfr::io(s1);  // Outputs: {0, 1, H, e, l, l, o, , ,
///     0, 6, 7, 8, 9, 10, 11}
/// \endcode
///
...
using enable_not_ostreamable_t = std::enable_if_t<
    not_appliable<ostreamable_detector, Stream &,
                  const std::remove_reference_t<Type> &>::value,
    Stream &>;

template <class Stream, class Type>
using enable_not_istreamable_t = std::enable_if_t<
    not_appliable<istreamable_detector, Stream &, Type &>::value, Stream &>;
...
/// IO manipulator to read/write \aggregate `value` using its IO stream
/// operators or using \forcedlink{io_fields} if operators are not available.
///
/// \b Example:
/// \code
