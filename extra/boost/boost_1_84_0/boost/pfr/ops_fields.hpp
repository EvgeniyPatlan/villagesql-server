...
///
/// \returns `L > R || (L == R && tuple_size_v<T> > tuple_size_v<U>)`, where `L`
/// and `R` are the results of calling `std::tie` on first `N` fields of `lhs`
/// and
// `rhs` respectively; `N` is `std::min(tuple_size_v<T>, tuple_size_v<U>)`.
template <class T, class U>
...
///
/// \returns `L > R || (L == R && tuple_size_v<T> >= tuple_size_v<U>)`, where
/// `L` and `R` are the results of calling `std::tie` on first `N` fields of
/// `lhs` and
// `rhs` respectively; `N` is `std::min(tuple_size_v<T>, tuple_size_v<U>)`.
template <class T, class U>
...
std::size_t hash_fields(const T &x) {
  ...
  return detail::hash_impl<0, fields_count_val>::compute(
      detail::tie_as_tuple(x));
#else
  std::size_t result = 0;
