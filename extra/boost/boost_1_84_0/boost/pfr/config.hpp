...
/// Contains all the macros that describe Boost.PFR configuration, like
/// BOOST_PFR_ENABLED
///
/// \note This header file doesn't require C++14 Standard and supports all C++
/// compilers, even pre C++14 compilers (C++11, C++03...).

// Reminder:
//  * MSVC++ 14.2 _MSC_VER == 1927 <- Loophole is known to work (Visual Studio
//  ????)
//  * MSVC++ 14.1 _MSC_VER == 1916 <- Loophole is known to NOT work (Visual
//  Studio 2017)
...
#if (defined(_MSC_VER) && \
     _MSC_VER < 1916)  ///< in Visual Studio 2017 v15.9 PFR library with classic
                       ///< engine normally works
#define BOOST_PFR_NOT_SUPPORTED 1
#endif
#endif

#ifndef BOOST_PFR_USE_STD_MAKE_INTEGRAL_SEQUENCE
// Assume that libstdc++ since GCC-7.3 does not have linear instantiation depth
// in std::make_integral_sequence
...
#else
// There is no way to detect potential ability to be reflectable without
// std::is_aggregare
...
#define BOOST_PFR_CORE_NAME_PARSING                                    \
  (sizeof("auto __cdecl boost::pfr::detail::name_of_field_impl<") - 1, \
   sizeof(">(void) noexcept") - 1, backward("->"))
#elif defined(__clang__)
#define BOOST_PFR_CORE_NAME_PARSING                                          \
  (sizeof(                                                                   \
       "auto boost::pfr::detail::name_of_field_impl() [MsvcWorkaround = ") - \
       1,                                                                    \
   sizeof("}]") - 1, backward("."))
#elif defined(__GNUC__)
#define BOOST_PFR_CORE_NAME_PARSING                                        \
  (sizeof("consteval auto boost::pfr::detail::name_of_field_impl() [with " \
          "MsvcWorkaround = ") -                                           \
       1,                                                                  \
   sizeof(")]") - 1, backward("::"))
#else
