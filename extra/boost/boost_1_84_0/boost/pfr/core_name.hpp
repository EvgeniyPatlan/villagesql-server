...
/// Contains functions \forcedlink{get_name} and \forcedlink{names_as_array} to
/// know which names each field of any \aggregate has.
///
/// \fnrefl for details.
///
/// \b Synopsis:
...
///     boost::pfr::names_as_array<my_struct>(); assert(a[0] == "i");
/// \endcode
template <class T>
constexpr
#ifdef BOOST_PFR_DOXYGEN_INVOKED
  ...
  return detail::make_stdarray_from_tietuple(
      detail::tie_as_names_tuple<T>(),
      detail::make_index_sequence<tuple_size_v<T>>(), 1L);
}
