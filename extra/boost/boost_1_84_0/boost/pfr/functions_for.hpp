...
/// Contains BOOST_PFR_FUNCTIONS_FOR macro that defined comparison and stream
/// operators for T along with hash_value function.
/// \b Example:
/// \code
...
///     assert(s1 < s2);
///     std::cout << s1 << std::endl; // Outputs: {0, 1, H, e, l, l, o, , , 0,
///     6, 7, 8, 9, 10, 11}
/// \endcode
///
/// \podops for other ways to define operators and more details.
...
#define BOOST_PFR_FUNCTIONS_FOR(T)                                            \
  ...
  template <class Char, class Traits>                                         \
  BOOST_PFR_MAYBE_UNUSED inline ::std::basic_istream<Char, Traits> &          \
