...
# Package the VillageSQL production server tarball from an already-built (and
...
PACKAGE_NAME="villagesql-server-${VSQL_VERSION}-${PLATFORM}-${ARCH}"
TARBALL_NAME="${PACKAGE_NAME}.tar.gz"
STAGING_DIR="${TMPDIR:-/tmp}/vsql_prod_staging_$$"
...
log_step "VillageSQL Production Server Packaging"
echo ""
...
# Step 1: Run CPack to produce a base package (no Test components)
log_step "Step 1: Generating base package with CPack..."
cd "$BUILD_DIR"

CPACK_COMPONENTS="Client;Server;Server_Scripts;SharedLibraries;SupportFiles;Readme;Info;ExampleVebs"
...
# Step 2: Extract into staging directory under the final package name
log_step "Step 2: Extracting base package..."
mkdir -p "$STAGING_DIR/$PACKAGE_NAME"
cd "$STAGING_DIR/$PACKAGE_NAME"
tar xzf "$BASE_TARBALL"

# Step 3: Drop unwanted binaries from bin/
log_step "Step 3: Removing unwanted binaries..."
UNWANTED_BINS=(
    mysql_client_test
...
# Step 4: Remove non-English language directories from share/
log_step "Step 4: Removing non-English share/ language directories..."
if [[ -d "share" ]]; then
    find share -mindepth 1 -maxdepth 1 -type d \
        ! -name "charsets" \
        ! -name "english" \
        -exec rm -rf {} \; 2>/dev/null || true
    log_info "Kept share/charsets, share/english"
fi

# Step 5: Remove ALL test/example plugins from lib/plugin/
log_step "Step 5: Removing test/example plugins..."
...
# Step 6: Copy VEB files
if [[ -n "$VEB_DIR" && -d "$VEB_DIR" ]]; then
    log_step "Step 6: Copying VEB files from $VEB_DIR..."
    mkdir -p lib/veb
    VEB_COUNT=0
...
log_step "Step 7: Adding convenience scripts..."
...
log_step "Step 8: Creating version file..."
cat > VERSION <<EOF
VillageSQL Version: $VSQL_VERSION
Platform: $PLATFORM-$ARCH
Build Date: $(date -u +"%Y-%m-%d %H:%M:%S UTC")
Package Type: Production Server
EOF

# Step 9: Create final tarball
log_step "Step 9: Creating final tarball..."
cd "$STAGING_DIR"
