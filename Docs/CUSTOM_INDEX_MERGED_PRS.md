...
### Open: PR #809 — "Custom Index: Load index profiles"

Opened 2026-07-13, updated 2026-07-30. Debarun's only open
indexing PR right now.

Loads profile metadata for key columns so InnoDB can invoke
profile callbacks (distance, compare) the extension declared.
This is what would eventually make `profile_fn`/`helper_fn`
non-stubs — but the call site would still be Debarun's dict
layer, not our optimizer hook.

**Watch for:** whether it turns our runtime's stub
`dummy_profile_fn` into a real dispatcher. If it does, our
runtime's `.helper_fn = dummy_profile_fn` install can be
deleted after this PR merges.

Touches: `ddl0ddl.cc`, `dict0dd.cc`, `ha_innodb.cc`,
`handler0alter.cc`, `ddl0ddl.h`, `custom_index.{cc,h}`,
`types/util.{cc,h}`.
...
| 4 | `profile_fn`/`helper_fn`/`key_len_fn` are stubs         | #809 (open) may partially close.                     |
...
| 8 | No cost/cardinality/stats for custom indexes            | No change.                                           |
...
Debarun's cadence remains roughly on track (5-6 indexing PRs
in the ~4 weeks since our July rebase, including two bug fixes
and one substantial new capability in #808). Gap #1 was the
biggest single overhang; #808 makes real progress on it.

`mark_delete`/`purge` invocation is the next obvious PR to
expect — Debarun's own TODO in `Custom_index::insert` calls it
out. Watch for those.
