...
// TESTTYPE implementation with full precision (%.17g format)
...
bool decode_testtype_full(const unsigned char *buffer, size_t buffer_size,
                          char *to, size_t to_buffer_size, size_t *to_length) {
  double real, imag;
  memcpy(&real, buffer, 8);
  memcpy(&imag, buffer + 8, 8);
  *to_length = snprintf(to, to_buffer_size, "(%.17g,%.17g)", real, imag);
  return false;
}
...
VEF_GENERATE_ENTRY_POINTS(
    make_extension(VEF_EXTENSION_NAME, "0.0.1-devtest")
        ...
        .func(make_func("testtype_to_string")
                  .to_string<&decode_testtype_full>(TESTTYPE)))
