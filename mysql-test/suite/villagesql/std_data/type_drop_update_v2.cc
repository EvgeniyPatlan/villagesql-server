...
// type_drop_update_v2.cc - v2: drops GAUGE, keeps COUNTER, adds SCORE.
// GAUGE has no dependent columns so the drop is allowed.
// COUNTER is retained with the same storage layout.
// SCORE is a new type.
...
static bool decode_score(const unsigned char *buffer, size_t buffer_size,
                         char *to, size_t to_buffer_size, size_t *to_length) {
  int val = 0;
  memcpy(&val, buffer, 4);
  *to_length = snprintf(to, to_buffer_size, "score:%d", val);
  return false;
}
...
VEF_GENERATE_ENTRY_POINTS(
    make_extension()
        ...
        .func(make_func("score_val").to_string<&decode_score>(SCORE)))
