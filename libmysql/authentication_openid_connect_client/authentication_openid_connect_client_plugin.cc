...
// TODO(villagesql-rebase): byte-faithful backport of the MySQL 9.1+ client
// plugin libmysql/authentication_openid_connect_client/ into this 8.4-based
// fork, so the native OIDC client path (and hermetic mtr tests for the
// vsql_oauth2 extension) work without a newer client. The server-side
// authentication_openid_connect plugin is Enterprise-only and intentionally NOT
// backported -- the vsql_oauth2 extension is our server-side OIDC verifier. On a
// rebase to a MySQL version that already ships this file, drop this copy and use
// upstream's. Keep this source byte-identical to upstream (do not villint /
// reformat) so that reconciliation stays trivial.
...
static int openid_connect_authentication_client_plugin(MYSQL_PLUGIN_VIO *vio,
  ...
  // TODO(villagesql-rebase): upstream (9.1+) also treats a TLS-over-TCP
  // connection as secure via vio_info.is_tls_established. That
  // MYSQL_PLUGIN_VIO_INFO field does not exist in this 8.4 tree, and this
  // CLIENT_ONLY/MODULE_ONLY plugin cannot call libmysqlclient (e.g.
  // mysql_get_ssl_cipher) to detect TLS another way. So this backport accepts
  // ONLY unix socket + shared memory as secure transports; a TLS-over-TCP
  // connection is rejected here (fail closed) rather than allowed. Restore the
  // is_tls_established check when rebasing onto a version that provides it.
