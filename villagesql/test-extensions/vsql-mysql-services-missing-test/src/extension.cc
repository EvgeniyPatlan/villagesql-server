...
::vsql::mysql::RequiredService<SERVICE_TYPE(vsql_intentionally_missing)>
    missing{"vsql_intentionally_missing"};
...
VEF_GENERATE_ENTRY_POINTS(
    make_extension()
        .with_airlock(missing)
        .func(make_func<&noop>("noop").returns(INT).no_params().build()))
