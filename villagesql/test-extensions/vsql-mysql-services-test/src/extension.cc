...
// vsql_mysql_services_test extension: exercises the MySQL Services airlock
// bridge end-to-end.
//
// Declares three RequiredServices (component_status, reader, writer), all
// sharing a single airlock channel. Each must be wired up by the airlock
// before any VDF runs. If any fails to acquire, extension load fails.
//
// VDFs:
...
VSQL_DECLARE_SERVICE(keyring_writer, writer);
...
void keyring_store(StringArg data_id, StringArg auth_id, StringArg value,
  ...
  bool err = writer->store(
      did, aid, reinterpret_cast<const unsigned char *>(val.data()),
      val.size(), "SECRET");
  out.set(err ? 1 : 0);
}
