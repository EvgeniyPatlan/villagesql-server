...
std::optional<TypeDescriptor> build_type_descriptor_v3(
  ...
  if (td->persisted_length == -1) {
    // Variable-length type: max_persisted_length bounds the backing field and
    ...
    // variable-length type whose length is decided per value (e.g. a roaring
    // bitmap) needs only max_persisted_length.
