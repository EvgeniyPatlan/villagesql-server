...
struct CapabilityValue {
  ...
  // Optional. Called after receive() for each extension that requires this
  // capability. Receives the extension name and extension_data pointer.
  // NULL for capabilities that need no post-receive setup.
...
static bool default_compat_fn(const vef_required_capability_t &req,
  ...
  if (entry->vtable_abi_hash != req.vtable_abi_hash) {
    error_message = std::string("capability vtable ABI mismatch: ") + req.name;
    return false;
  }
