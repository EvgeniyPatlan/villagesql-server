...
// binlog-XID assert on the 2nd DDL. Auto_THD's ctor hijacks current_thd via
// store_globals(), so capture it first and restore after (else the rest of
// acl_authenticate runs on a dangling current_thd).
...
bool method_wants_auto_grant(std::string_view method_name) {
  std::lock_guard<std::mutex> lock(g_mu);
  // Gated by the login's OWN method (the one that staged the roles), queried
  // live so it reflects the extension's runtime sysvar. Independent of the
  // unknown-account opt-in above.
  const std::string normalized =
      normalize_extension_name(std::string(method_name));
  for (const auto &m : g_methods) {
    if (m.method_name == normalized) {
...
void maybe_apply_vef_role_grants(MPVIO_EXT *mpvio, const char *acl_user_authid,
  ...
  // Gate on THIS login's method opting into auto-grant (independent of the
  // unknown-account opt-in). Off by default: the token stays activate-only, the
  // DBA owns grants. The method name is the account's plugin, set at handshake.
  const char *const method = mpvio->acl_user_plugin.str;
  ...
  // GRANT each staged role additively (never revoke; a role no longer claimed
  // is retained -- authoritative reconcile is a separate, deferred task). Roles
  // must pre-exist as DB roles; an ungrantable one is skipped. maybe_apply_vef_
  // auth_state's grant-checked activation then finds and activates them.
